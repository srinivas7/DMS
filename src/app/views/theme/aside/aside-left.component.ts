...
export class AsideLeftComponent implements OnInit, AfterViewInit {
	 ...
	 * @param menuAsideService: MenuAsideService
	...
	isMenuRootItemIsActive(item): boolean {
		...
		for (const subItem of item.policies) {
	...
	getItemCssClasses(item) {
		...
		if (objectPath.get(item, 'policies')) {
			classes += ' kt-menu__item--submenu';
		}

		if (!item.policies && this.isMenuItemIsActive(item)) {
	...
	getItemAttrSubmenuToggle(item) {
		...
		if (objectPath.get(item, 'toggle') === 'click') {
			toggle = 'click';
		} else if (objectPath.get(item, 'policies.type') === 'tabs') {
			toggle = 'tabs';
		} else {
