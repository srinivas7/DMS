...
// Services and Models
import { CustomerModel, CustomersDataSource, OneCustomerDeleted, ManyCustomersDeleted, CustomersStatusUpdated, CustomersService } from '../../../../../../core/e-commerce';
// Components
...
export class CustomersListComponent implements OnInit, OnDestroy {
	...
	displayedColumns = ['id', 'policyName', 'displayName', 'description', 'url', 'actions'];
	...
	ngOnInit() {
		...
		**/
		// const paginatorSubscriptions = merge(this.sort.sortChange, 0).pipe(
	...
	loadCustomers() {
		...
		this.customersService.getAllCustomers(queryParams).subscribe((result) => {
	...
	deleteCustomer(_item: CustomerModel) {
		...
		dialogRef.afterClosed().subscribe(res => {
			...
			this.layoutUtilsService.showActionNotification(_deleteMessage, MessageType.Delete).afterOpened().subscribe((res) => {
				this.page = (this.customersResult.length === 1) ? this.page - 1 : this.page;
				this.loadCustomers();
			});
	...
	fetchCustomers() {
		...
		this.selection.selected.forEach(elem => {
			messages.push({
				text: `${elem.displayName}, ${elem.policyName}`,
				id: elem.id.toString(),
				status: elem.status
			});
		});
