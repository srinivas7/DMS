...
export class ForgotPasswordComponent implements OnInit, OnDestroy {
	...
	submit() {
		...
		const employeeid = controls.employeeid.value;
		this.authService.requestPassword(employeeid).pipe(
			tap(response => {
				if (response) {
					this.authNoticeService.setNotice(this.translate.instant('AUTH.FORGOT.SUCCESS'), 'success');
					this.router.navigateByUrl('/auth/login');
				} else {
					this.authNoticeService.setNotice(this.translate.instant('AUTH.VALIDATION.NOT_FOUND', {name: this.translate.instant('EmployeeID')}), 'danger');
				}
			}),
