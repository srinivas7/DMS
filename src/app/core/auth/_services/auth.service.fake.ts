...
export class AuthService {
    ...
    login(empname: string, password: string): Observable<User> {
        if (!empname || !password) {
            return of(null);
        }
        ...
        return  this.getAllUsers().pipe(
            map((result: User[]) => {
                ...
                const user = find(result, (item: User) => {
                    return (item.empname === empname && item.password === password);
                });
    ...
    requestPassword(empname: string): Observable<any> {
